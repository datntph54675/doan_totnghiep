...
<head>
    ...
    <style>
        ...
        *{ box-sizing:border-box }
        ...
        input[type=email]{ width:100%; padding:11px 12px; border:1px solid #e6e9ef; border-radius:8px; font-size:14px; background:#fbfdff }
...
<body>
    <div class="wrap">
        <div class="card">
            <h2 style="margin:0 0 6px">Quên mật khẩu</h2>
            <div class="muted">Nhập email của bạn để nhận link đặt lại mật khẩu.</div>

            @if (session('status'))
            <div class="success" style="margin-top:12px">{{ session('status') }}</div>
            @endif
            ...
            <form method="POST" action="{{ route('password.email') }}" style="margin-top:12px">
                @csrf
                <div class="field">
                ...
                <div class="actions">
                    ...
                    <button class="btn" type="submit">Gửi link reset</button>
                </div>
            </form>

            <div class="muted">
                Lưu ý: nếu đang chạy local và `MAIL_MAILER=log`, email sẽ nằm trong `storage/logs/laravel.log`.
            </div>
        </div>
    </div>
</body>
