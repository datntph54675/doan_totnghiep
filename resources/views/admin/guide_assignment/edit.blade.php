...
@section('content')
    <div class="container">
        ...
        <form action="{{ route('admin.guide-assignments.update', $guideAssignment->id) }}" method="POST"
                    ...
                    <select name="schedule_id" id="schedule_id"
                        ...
                        @foreach($schedules as $schedule)
                            <option value="{{ $schedule->schedule_id }}"
                                @selected($guideAssignment->schedule_id == $schedule->schedule_id)>
                                {{ $schedule->tour->name }} -
            ...
            <div class="mb-3">
                ...
                <textarea name="note" id="note" class="form-control @error('note') is-invalid @enderror" rows="3"
                    placeholder="Nhập ghi chú nếu có...">{{ old('note', $guideAssignment->note) }}</textarea>
                @error('note')
                    <span class="invalid-feedback d-block">{{ $message }}</span>
                @enderror
            </div>
