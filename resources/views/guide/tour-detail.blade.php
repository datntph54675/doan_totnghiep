    ...
    <div class="card" style="margin-bottom:20px">
        ...
        <div class="card-body">
            <div style="display:grid; grid-template-columns:repeat(auto-fit,minmax(200px,1fr)); gap:20px">
                <div>
                    <div
                        style="font-size:12px;font-weight:700;color:var(--text-muted);text-transform:uppercase;letter-spacing:.5px;margin-bottom:6px">
                        Ngày khởi hành</div>
            ...
            @if($schedule->tour->description ?? false)
                <div
                    style="margin-top:20px;padding:14px 16px;background:#f0f9ff;border:1px solid #bae6fd;border-radius:8px;font-size:14px;color:#0c4a6e">
            ...
            @if($schedule->notes)
                <div
                    style="margin-top:12px;padding:14px 16px;background:#fefce8;border:1px solid #fde68a;border-radius:8px;font-size:14px;color:#92400e">
                    📝 {{ $schedule->notes }}
                </div>
            @endif

            <div style="display:flex;gap:10px;margin-top:20px;flex-wrap:wrap">
                <a href="{{ route('guide.attendance', $schedule->schedule_id) }}" class="btn btn-primary">✅ Điểm danh
                    khách</a>
                <a href="{{ route('guide.itinerary', $schedule->schedule_id) }}" class="btn btn-outline">🗓️ Quản lý lịch
                    trình</a>
                @if(($canMarkCompleted ?? false) === true)
                    <form action="{{ route('guide.tour.complete', $schedule->schedule_id) }}" method="POST"
                        onsubmit="return confirm('Xác nhận đã hoàn thành tour này?');">
                        @csrf
                        <button type="submit" class="btn btn-success">🏁 Xác nhận hoàn thành tour</button>
                    </form>
                @elseif(($assignment->status ?? null) === 'completed')
                    <span class="badge badge-success" style="display:inline-flex;align-items:center;padding:10px 12px;">Trạng
                        thái phân công: Hoàn thành</span>
                @endif
            </div>
        </div>
    </div>
        ...
        <div class="card-body" style="padding-top:12px">
            @if(($participants ?? collect())->count() > 0)
                ...
                @foreach($participantGroups as $groupMembers)
                    ...
                    @endphp
                    <div
                        style="border:1px solid var(--border);border-radius:12px;margin-bottom:12px;overflow:hidden;background:#fff;">
                        <div
                            style="display:flex;align-items:center;justify-content:space-between;gap:12px;padding:12px 14px;background:#f1f5f9;border-bottom:1px solid var(--border)">
                            ...
                            @foreach($groupMembers as $participant)
                                <div class="{{ !$participant->is_representative ? 'participant-child' : '' }}"
                                    ...
                                    <div style="display:flex;align-items:center;gap:14px;flex:1;min-width:0">
                                        <div
                                            style="width:38px;height:38px;border-radius:10px;background:linear-gradient(135deg,#6366f1,#8b5cf6);display:flex;align-items:center;justify-content:center;color:#fff;font-weight:700;font-size:14px;flex-shrink:0">
            ...
            <div class="card-body">
                @foreach($itineraries as $item)
                    <div style="display:flex;gap:16px;padding:14px 0;border-bottom:1px solid var(--border)">
                        <div
                            style="background:var(--primary-bg);color:var(--primary);padding:8px 14px;border-radius:8px;font-weight:700;font-size:13px;white-space:nowrap;height:fit-content">
        ...
        <div class="card">
            ...
            <div class="card-body">
                @foreach($feedbacks as $fb)
                    <div
                        style="padding:16px;background:#f8fafc;border-radius:10px;margin-bottom:12px;border:1px solid var(--border)">
