...
.booking-hero-float:nth-child(1) { width: 200px; height: 200px; top: -60px; right: 10%; animation-delay: 0s; }
.booking-hero-float:nth-child(2) { width: 120px; height: 120px; bottom: -30px; right: 25%; animation-delay: 2s; }
.booking-hero-float:nth-child(3) { width: 80px; height: 80px; top: 20px; left: 8%; animation-delay: 4s; }
...
.booking-breadcrumb a { color: rgba(255,255,255,0.75); transition: color 0.2s; }
.booking-breadcrumb a:hover { color: #fff; }
.booking-breadcrumb .sep { opacity: 0.5; }
.booking-hero-title {
    font-size: 30px;
    font-weight: 800;
    color: #fff;
...
.step-arrow { color: var(--border); font-size: 12px; margin: 0 4px; }
...
.bk-card:hover { box-shadow: 0 4px 24px rgba(0,0,0,0.1); }
.bk-card-head {
    padding: 20px 28px;
    display: flex;
    align-items: center;
...
.bk-card-head-text p { font-size: 12px; color: var(--text-light); }
...
.schedule-option:hover { border-color: var(--primary); background: var(--primary-light); }
.schedule-option.selected { border-color: var(--primary); background: #f0f8ff; }
.schedule-option.selected::before { opacity: 1; }
.schedule-option.disabled-opt { opacity: 0.5; cursor: not-allowed; background: #f8f9fa; }
.schedule-option input[type=radio] { display: none; }
.schedule-date-info { flex: 1; }
.schedule-date-main {
    font-size: 15px;
    font-weight: 700;
    color: var(--text-dark);
...
.schedule-date-arrow { color: var(--primary); font-size: 12px; }
.schedule-date-sub { font-size: 12px; color: var(--text-light); display: flex; align-items: center; gap: 12px; }
.schedule-spot-badge {
    padding: 4px 14px;
    border-radius: 20px;
    font-size: 12px;
...
.spot-ok { background: #ecfdf5; color: #059669; }
.spot-few { background: #fff7ed; color: #ea580c; }
.spot-none { background: #fef2f2; color: #dc2626; }
...
.people-selector:focus-within { border-color: var(--primary); }
.people-btn {
...
.people-btn:hover { background: var(--primary); color: #fff; }
.people-input {
    width: 80px;
    text-align: center;
    font-size: 18px;
...
.people-input::-webkit-inner-spin-button { -webkit-appearance: none; appearance: none; margin: 0; }
...
.spot-indicator-bar.ok { background: #f0fdf4; border-color: #bbf7d0; color: #15803d; }
.spot-indicator-bar.warn { background: #fff7ed; border-color: #fed7aa; color: #c2410c; }
.spot-indicator-bar.error { background: #fef2f2; border-color: #fecaca; color: #dc2626; }
.spot-progress { flex: 1; height: 6px; border-radius: 3px; background: rgba(0,0,0,0.1); overflow: hidden; }
.spot-progress-fill { height: 100%; border-radius: 3px; transition: width 0.4s ease; }
.ok .spot-progress-fill { background: #22c55e; }
.warn .spot-progress-fill { background: #f97316; }
.error .spot-progress-fill { background: #ef4444; }

/* ====== FORM FIELDS ====== */
.fi-group { margin-bottom: 20px; }
.fi-label {
    display: flex;
    align-items: center;
    gap: 6px;
...
.fi-control:focus { border-color: var(--primary); box-shadow: 0 0 0 4px rgba(0,102,204,0.1); }
.fi-control.prefilled { background: #f8fcff; border-color: rgba(0,102,204,0.3); }
.fi-row { display: grid; grid-template-columns: 1fr 1fr; gap: 18px; }
@media (max-width: 560px) { .fi-row { grid-template-columns: 1fr; } }
.fi-error { color: #ef4444; font-size: 12px; margin-top: 6px; font-weight: 500; display: flex; align-items: center; gap: 4px; }
.autofill-notice {
    display: flex;
    align-items: center;
    gap: 8px;
...
.bk-info-row:last-of-type { border-bottom: none; }
.bk-info-label { color: var(--text-light); display: flex; align-items: center; gap: 6px; }
.bk-info-value { font-weight: 600; color: var(--text-dark); }
.bk-price-box {
...
.bk-price-row.total .label { font-size: 15px; font-weight: 700; color: var(--text-dark); }
.bk-price-row.total .amount { font-size: 22px; font-weight: 800; color: var(--primary); }
.bk-trust {
    display: flex;
    flex-direction: column;
    gap: 8px;
...
.bk-trust-item i { color: var(--success); font-size: 13px; }
...
.bk-empty h4 { font-size: 16px; font-weight: 700; color: var(--text-dark); margin-bottom: 6px; }
.bk-empty p { font-size: 14px; color: var(--text-light); }
                ...
                <div class="bk-card-body">
                    @if($schedules->count() > 0)
                        <div class="schedule-grid" id="scheduleGrid">
                            ...
                            @endphp
                            <label class="schedule-option {{ $spots <= 0 ? 'disabled-opt' : '' }} {{ old('schedule_id') == $s->schedule_id ? 'selected' : '' }}"
                                   data-id="{{ $s->schedule_id }}"
                                   data-spots="{{ $spots }}"
                                   for="sch_{{ $s->schedule_id }}">
                                <input type="radio"
                                    id="sch_{{ $s->schedule_id }}"
                                    name="schedule_id"
                                    value="{{ $s->schedule_id }}"
                                ...
                                <div class="schedule-date-info">
                                    <div class="schedule-date-main">
                                        <i class="fa-solid fa-calendar-day" style="color:var(--primary);font-size:14px"></i>
                                        {{ $s->start_date->format('d/m/Y') }}
                                        <span class="schedule-date-arrow">→</span>
                                        {{ $s->end_date->format('d/m/Y') }}
                                    </div>
                                    <div class="schedule-date-sub">
                                        <span><i class="fa-solid fa-sun" style="color:#f59e0b"></i> {{ $duration }} ngày {{ $duration - 1 }} đêm</span>
                                        @if($s->meeting_point)
                                        <span><i class="fa-solid fa-map-pin" style="color:#ef4444"></i> {{ $s->meeting_point }}</span>
                                        @endif
                                    </div>
                                </div>
                        ...
                        <div style="border-top: 1px solid #f1f5f9; padding-top: 20px; margin-top: 4px;">
                            ...
                            <div style="display:flex; align-items:center; gap:24px; flex-wrap:wrap;">
                                <div class="people-selector">
                                    ...
                                    <input type="number" name="num_people" id="numPeople"
                                        class="people-input" value="{{ old('num_people', 1) }}" min="1" max="50" readonly>
                                    <button type="button" class="people-btn" id="btnPlus">+</button>
                                </div>
                                <div style="font-size:13px; color:var(--text-light)">
                                    <div id="pricePerPerson">Giá/người: <strong>{{ number_format($tour->price, 0, ',', '.') }} ₫</strong></div>
                                </div>
                            </div>
                            ...
                            @error('num_people')
                            <div class="fi-error" style="margin-top:10px"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</div>
                            @enderror
                        </div>
                    @else
                        <div class="bk-empty">
                            ...
                            <p>Hiện chưa có lịch khởi hành nào khả dụng cho tour này.<br>Vui lòng quay lại sau hoặc liên hệ chúng tôi.</p>
                        </div>
                    @endif
                </div>
            </div>
                ...
                <div class="bk-card-body">
                    ...
                    <div class="fi-row">
                        <div class="fi-group">
                            ...
                            <input type="text" name="fullname" class="fi-control {{ $user ? 'prefilled' : '' }}"
                                value="{{ old('fullname', $user->fullname ?? '') }}"
                                placeholder="Nguyễn Văn An" required>
                            @error('fullname')<div class="fi-error"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</div>@enderror
                        </div>
                        <div class="fi-group">
                            ...
                            @error('gender')<div class="fi-error"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</div>@enderror
                        </div>
                    </div>
                    ...
                    <div class="fi-row">
                        <div class="fi-group">
                            ...
                            <input type="tel" name="phone" class="fi-control {{ $user ? 'prefilled' : '' }}"
                                value="{{ old('phone', $user->phone ?? '') }}"
                                placeholder="0901 234 567" required>
                            @error('phone')<div class="fi-error"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</div>@enderror
                        </div>
                        <div class="fi-group">
                            <div class="fi-label">
                                Email <span class="req">*</span>
                            </div>
                            <input type="email" name="email" class="fi-control {{ $user ? 'prefilled' : '' }}"
                                value="{{ old('email', $user->email ?? '') }}"
                                placeholder="example@email.com" required>
                            @error('email')<div class="fi-error"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</div>@enderror
                        </div>
                    </div>
                    ...
                    <div class="fi-row">
                        <div class="fi-group">
                            ...
                            <input type="date" name="birthdate" class="fi-control"
                                value="{{ old('birthdate') }}">
                            @error('birthdate')<div class="fi-error"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</div>@enderror
                        </div>
                        <div class="fi-group">
                            <div class="fi-label">
                                Số CCCD / Hộ chiếu
                            ...
                            @error('id_number')<div class="fi-error"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</div>@enderror
                        </div>
                    </div>
        ...
        <div class="bk-summary-card">
            ...
            <div class="bk-summary-body">
                ...
                <div class="bk-info-row">
                    ...
                    <span class="bk-info-value" style="font-family:monospace;letter-spacing:0.5px">VT-{{ str_pad($tour->tour_id, 4, '0', STR_PAD_LEFT) }}</span>
                </div>
                <div class="bk-info-row">
                ...
                <div class="bk-price-box">
                    ...
                    <div class="bk-price-row total">
                        ...
                        <span class="amount" id="sidebarTotal">{{ number_format($tour->price, 0, ',', '.') }} ₫</span>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.schedule-option:not(.disabled-opt)').forEach(function(label) {
        label.addEventListener('click', function () {
            document.querySelectorAll('.schedule-option').forEach(l => l.classList.remove('selected'));
    ...
    btnMinus && btnMinus.addEventListener('click', function () {
        ...
        if (v > 1) { numInput.value = v - 1; updateAll(); }
    });
    btnPlus && btnPlus.addEventListener('click', function () {
        const v = parseInt(numInput.value) || 1;
        if (v < 50) { numInput.value = v + 1; updateAll(); }
    });
