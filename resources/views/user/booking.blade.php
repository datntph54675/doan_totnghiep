...
<div class="container" style="margin-top: 20px;">
    ...
    <div
        style="background: #fff7ed; color: #c2410c; border: 1px solid #fed7aa; padding: 14px 20px; border-radius: 12px; margin-top: 14px; font-size: 14px; display: flex; gap: 10px; align-items: flex-start;">
        <i class="fa-solid fa-triangle-exclamation" style="margin-top: 2px;"></i>
        <div>{{ $errors->first('companions') }}</div>
    </div>
    @endif
</div>
...
<div class="booking-layout">
    ...
    <div>
        <form method="POST" action="{{ route('user.booking.store', $tour->tour_id) }}" id="bookingForm">
            ...
            <div class="bk-card" id="companionsCard" style="display:none;">
                <div class="bk-card-head">
                    <div class="bk-card-icon">🧾</div>
                    <div class="bk-card-head-text">
                        <h3>Thông tin người đi cùng</h3>
                        <p>Khi đặt từ 2 người trở lên, cần nhập đầy đủ thông tin từng người</p>
                    </div>
                </div>
                <div class="bk-card-body">
...
<div id="booking-meta" data-price="{{ $tour->price }}" data-old-companions='@json(old("companions", []))'
    style="display:none">
</div>
@endsection
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const companionsCard = document.getElementById('companionsCard');
        const companionsContainer = document.getElementById('companionsContainer');
        const oldCompanions = JSON.parse(document.getElementById('booking-meta').dataset.oldCompanions || '[]');
        ...
        function collectCompanionValues() {
            ...
            const items = Array.from(companionsContainer.querySelectorAll('.companion-item'));

            return items.map((item) => ({
                fullname: item.querySelector('[data-field="fullname"]')?.value || '',
        ...
        function renderCompanionFields(forceOld = false) {
            if (!companionsCard || !companionsContainer || !numInput) {
                return;
            }

            const selectedPeople = parseInt(numInput.value) || 1;
            const companionCount = Math.max(0, selectedPeople - 1);

            companionsCard.style.display = companionCount > 0 ? 'block' : 'none';
            ...
            const currentValues = forceOld ? [] : collectCompanionValues();
            const sourceValues = currentValues.length ? currentValues : oldCompanions;
            let html = '';

            for (let i = 0; i < companionCount; i++) {
                const value = sourceValues[i] || {};
                html += `
                        <div class="companion-item" data-index="${i}">
                            <div class="companion-head"><i class="fa-solid fa-user-group" style="color:var(--primary)"></i> Người đi cùng #${i + 1}</div>
                            <div class="fi-row">
                                <div class="fi-group">
                                    <div class="fi-label">Họ và tên <span class="req">*</span></div>
                                    <input type="text" class="fi-control" name="companions[${i}][fullname]" data-field="fullname" value="${escapeHtml(value.fullname || '')}" placeholder="Nguyễn Văn B" required>
                                </div>
                                <div class="fi-group">
                                    <div class="fi-label">Giới tính <span class="req">*</span></div>
                                    <select class="fi-control" name="companions[${i}][gender]" data-field="gender" required>
                                        <option value="">-- Chọn giới tính --</option>
                                        <option value="Nam" ${(value.gender || '') === 'Nam' ? 'selected' : ''}>Nam</option>
                                        <option value="Nữ" ${(value.gender || '') === 'Nữ' ? 'selected' : ''}>Nữ</option>
                                        <option value="Khác" ${(value.gender || '') === 'Khác' ? 'selected' : ''}>Khác</option>
                                    </select>
                                </div>
                            </div>
                            <div class="fi-row">
                                <div class="fi-group">
                                    <div class="fi-label">Ngày sinh <span class="req">*</span></div>
                                    <input type="date" class="fi-control" name="companions[${i}][birthdate]" data-field="birthdate" value="${escapeHtml(value.birthdate || '')}" required>
                                </div>
                                <div class="fi-group">
